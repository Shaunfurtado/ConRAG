...
export class RAGSystem {
  ...
  // Modify to return metadata properly
async getDocumentNames(sessionId: string): Promise<{ file_name: string, metadata: { source: string } }[]> {
  const documentNames = await this.databaseService.getDocumentNames(sessionId);
  return documentNames.map(doc => ({
    file_name: doc.file_name,
    metadata: { source: doc.file_name }  // Add the metadata structure with the source as the file name
  }));
}
